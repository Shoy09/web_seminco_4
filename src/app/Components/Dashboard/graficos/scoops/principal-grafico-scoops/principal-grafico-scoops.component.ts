...
@Component({
  ...
  imports: [CommonModule, FormsModule, ResumenComponent, CucharasLaborComponent, CucharasEquipoComponent, RendimientoEquipoComponent, DemorasOperativasComponent, DemorasInoperativasComponent, HorasMantenimientoComponent, TonelajeEquipoComponent, TnHrEquipoComponent, HorometroDieselComponent, HorometroDieselTotalComponent, ScatterTurnosComponent, ScatterTurnosNocheComponent, HorasPrimerViajeComponent, ResumenEquiposComponent, MejoresOperadoresComponent, MejoresOperadoresGraficoComponent, ObservacionesComponent],
...
procesarResumenSC() {
  ...
  const viajesPorEquipo = conteoEquipos > 0
    ? totalCucharas / conteoEquipos
    : 0;
...
ProcesarDM_SC_UTI_SC() {
  ...
  this.operacionesFiltradas.forEach(op => {
    ...
    const mantenimiento = this.calcularDuracionEstado(registros, 'MANTENIMIENTO');
    grupo.horas_mantenimiento += mantenimiento;

    // 3. DEMORA solo código 206 (igual que Power BI)
    const demoras206 = this.calcularDuracionEstado(registros, 'DEMORA', '206');
  ...
  const resultado = Array.from(mapa.values()).map(item => {
    ...
    const HorasMantenimiento = item.horas_mantenimiento + item.horas_demoras_206;

    // Ajuste: si HorasMantenimiento = 0, usar 0.5 * n_operaciones
    const HorasMantenimientoAjustado = HorasMantenimiento === 0 
    ...
    if (HayRegistros && HorasProgramadas > 0) {
      DM_SC = (HorasProgramadas - HorasMantenimientoAjustado) / HorasProgramadas;
    }
    ...
    return {
      ...
      HorasMantenimientoAjustado: Number(HorasMantenimientoAjustado.toFixed(2)),
...
procesarDemorasInoperativas() {
  ...
  // 🔥 Lista de códigos permitidos (solo estos 8)
  const codigosPermitidos = new Set([
    "206","209", "210", "212", "213", "214", "215", "216"
  ]);
...
ProcesarTonelajePorHoraSC() {
  ...
  const resultado = Array.from(mapa.values()).map(item => {
    ...
    const Tn_h_SC =
      item.horas_operativo > 0
        ? item.tonelaje / item.horas_operativo
        : 0;
...
ProcesarDiferenciaDieselTotal() {
  ...
  const resultado = {
    ...
    PromedioPorOperacion: operaciones > 0
...
ProcesarContViajesSC() {
  ...
  this.operacionesFiltradas.forEach(op => {
    ...
    const laborFinal = laborConcat && laborConcat.trim() !== ''
      ? laborConcat.trim()
      : 'SIN_LABOR';
...
ProcesarOperadorTurnoSC() {
  ...
  const resultado = Array.from(mapa.values()).map(item => {
    ...
    const tn_h =
      item.horasOperativo > 0
        ? item.tonelaje / item.horasOperativo
        : 0;
...
procesarLaborFRDetallado() {
  ...
  this.operacionesFiltradas.forEach(op => {
    ...
    const modelo = (op.equipo && op.n_equipo)
      ? `${op.equipo}-${op.n_equipo}`
      : 'SIN_EQUIPO';
    ...
    registrosArray.forEach(r => {
      ...
      const laborFinal = concat && concat.trim() !== ''
        ? concat.trim()
        : 'SIN_LABOR';
