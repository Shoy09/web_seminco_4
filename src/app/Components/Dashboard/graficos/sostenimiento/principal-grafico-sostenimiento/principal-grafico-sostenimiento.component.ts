    ...
    try {
      ...
      if (Array.isArray(registrosArray)) {
        ...
        for (const registro of registrosArray) {
          ...
          const laborSOS = `${opReg.tipo_labor || ''}${opReg.labor || ''}${opReg.ala || ''}`.trim();
...
private obtenerSeccionLabor(opReg: any, mapaPlanes: Map<string, any>): string {
...
ProcesarMHrEquipo() {
  ...
  const resultado = Array.from(mapa.values()).map(item => {
    ...
    const mh = item.horasPercusion > 0
      ? item.metros / item.horasPercusion
      : 0;
...
ProcesarMetrosPerforadosEquipo() {
  ...
  const resultado = Array.from(mapa.entries()).map(([modeloEquipo, metros]) => ({
    modeloEquipo,
...
procesarIndicadores() {
  ...
  this.operacionesFiltradas.forEach(op => {
    ...
    registros.forEach(r => {
      ...
      const opData = (typeof r.operacion === 'object' && r.operacion !== null)
        ? r.operacion
        : {};
  ...
  mapa.forEach((d) => {
    ...
    const n_pernos_por_labor =
      d.n_labores_sostenidas > 0
        ? d.n_pernos / d.n_labores_sostenidas
        : 0;
