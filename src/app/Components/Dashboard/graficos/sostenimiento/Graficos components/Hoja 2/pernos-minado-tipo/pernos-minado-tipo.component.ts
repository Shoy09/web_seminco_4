...
@Component({
  selector: 'app-pernos-minado-tipo',
  standalone: true,
  imports: [NgxEchartsDirective],
  providers: [provideEchartsCore({ echarts })],
  templateUrl: './pernos-minado-tipo.component.html',
  styleUrl: './pernos-minado-tipo.component.css'
})
export class PernosMinadoTipoComponent implements OnChanges {
  ...
  generarGrafico() {
    ...
    // Agrupar por tipo de minado
    const agrupado: { [key: string]: number } = {};

    this.data.forEach(item => {
      const tipo = item.minado_tipo || item.tipo || 'SIN TIPO';
      const total = Number(item.totalPernos) || Number(item.total) || Number(item.cantidad) || 0;
  ...
  getOpcionesGrafico(data: any[]) {
    // Asignar color de la paleta de azules a cada dato
