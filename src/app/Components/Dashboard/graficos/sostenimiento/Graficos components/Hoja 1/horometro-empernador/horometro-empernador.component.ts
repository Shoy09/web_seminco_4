...
@Component({
  selector: 'app-horometro-empernador',
    standalone: true,
    imports: [NgxEchartsDirective],
    providers: [provideEchartsCore({ echarts })],
  templateUrl: './horometro-empernador.component.html',
  styleUrl: './horometro-empernador.component.css'
})
export class HorometroEmpernadorComponent implements OnChanges {
  ...
  updateChart(): void {
    ...
    const xAxisData = this.data.map(item => item.modeloEquipo || 'N/A');
    ...
    this.chartOptions = {
      ...
      tooltip: {
        ...
        formatter: (params: any) => {
          ...
          let result = `<strong>${item.modeloEquipo}</strong><br/>`;
