...
export class PrincipalScoopsComponent implements OnInit {
  ...
  onMenuChange(event: any) {
    ...
    switch (event.tipo) {
      ...
      case 'programaTrabajo': // 🔥 FALTABA ESTE
        this.programaTrabajoData = event.data;
        break;
    }
  }
...
validarCambios() {
  ...
  parsearNueva(nueva: any) {
    return {
      ...
      programa_trabajo: this.safeParse(nueva.programa_trabajo)
    };
  }
  ...
  reconstruirOperacion(): OperacionBase {
    return {
      ...
      capacidad: this.cardData.capacidad,
  ...
  this.normalizarTipoEquipo(this.cardData.tiposEquipo)
      ...
      programa_trabajo: JSON.stringify({
  n_cucharas_programado: this.programaTrabajoData?.n_cucharas_programado ?? 0,
  n_cucharas_realizado: this.programaTrabajoData?.n_cucharas_realizado ?? 0
}),
    };
  }
  ...
  formatearHorometros(data: any) {
    ...
    return {
      horometro: data.horometro ? map(data.horometro) : null,
    };
  }
  ...
  mapearDatos() {
    ...
    this.programaTrabajoData = this.mapearParaProgramaTrabajo(this.operacion);
  }
  ...
  mapearParaProgramaTrabajo(op: OperacionBase) {
  const p = this.parseSeguro(op.programa_trabajo);

  if (!p || typeof p !== 'object') return null;

  return {
    n_cucharas_programado: p.n_cucharas_programado ?? 0,
    n_cucharas_realizado: p.n_cucharas_realizado ?? 0
  };
}
}