...
export class PlanProduccionListComponent implements OnInit {
  ...
  planesProduccion: PlanProduccion[] = [];
  ...
  obtenerPlanesProduccion(anio: number, mes: string): void {
    this.planProduccionService
      .getPlanMensualByYearAndMonth(anio, mes)
      .subscribe(
        (planes) => {
          this.planesProduccion = planes;
        },
        (error) => {
  ...
  abrirDialogoCrear(): void {
    const dialogRef = this.dialog.open(CreatePlanProduccionComponent, {
      ...
      maxHeight: '90vh',
      autoFocus: false,
      panelClass: 'create-plan-producccion-responsive',
      data: {
        anio: this.anio,
        mes: this.mes,
      },
  ...
  async enviarDatosAlServidor(planes: PlanProduccion[]): Promise<void> {
    ...
    for (const [index, plan] of planes.entries()) {
      try {
        ...
        const response = await this.planProduccionService
          .createPlanProduccion(plan)
          .toPromise();
  ...
  editarPlan(plan: PlanProduccion): void {
    const dialogRef = this.dialog.open(EditPlanProduccionComponent, {
      ...
      maxHeight: '90vh',
      autoFocus: false,
      panelClass: 'edit-plan-producccion-responsive',
  ...
  verPlan(plan: any): void {
    this.dialog.open(PlanProduccionDetallesDialogComponent, {
      ...
      maxHeight: '90vh',
      autoFocus: false,
      panelClass: 'detalle-plan-producccion-responsive',
      data: plan,
    });
  }
  ...
  verDiferencias(plan: PlanProduccion): void {
    this.dialog.open(DialogDiferenciaPlanRealidadComponent, {
      ...
      maxHeight: '90vh',
      autoFocus: false,
      panelClass: 'diferencia-plan-producccion-responsive',
      data: {
